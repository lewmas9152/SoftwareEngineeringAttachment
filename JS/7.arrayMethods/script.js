...
function analyzeUserData(users) {
  const latestDate = users
    .flatMap((user) => user.posts.map((post) => new Date(post.timestamp)))
    .reduce((max, date) => (date > max ? date : max), new Date(0));

  const measureDate = new Date(latestDate);
  measureDate.setDate(measureDate.getDate() - 7);

  const activeUsers = users.filter((user) =>
    user.posts.some((post) => new Date(post.timestamp) > measureDate)
  );

  const popularPosts = activeUsers
    .map((user) => user.posts.filter((post) => post.likes >= 10))
    .flat();

  const totalLikes = popularPosts.reduce((sum, post) => sum + post.likes, 0);
  const averageLikes =
    activeUsers.length > 0 ? totalLikes / activeUsers.length : 0;
